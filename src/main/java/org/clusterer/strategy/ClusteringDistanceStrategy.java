...
public class ClusteringDistanceStrategy extends ClusteringStrategy {
	...
	public void generateCluster() {
		...
		try {
			...
			for (int i = 0; i < dataset.numInstances(); i++) {
				...
				if (clusterInstances.get(clusterer.clusterInstance(dataset
					...
					clusterInstances.get(
							clusterer.clusterInstance(dataset.instance(i)))
							.add(operationNames.get(i));
				}

				int instance = clusterer.clusterInstance(dataset.instance(i));
				if (hashClustering.get(instance) == null) { // gets the value
															// for an id)
			...
			for (Operation op : getOperations()) {
				...
				DataTypeNode output = new DataTypeNode(op.getOutput()
						.getElement());
				...
				if (!clusAux2.contains(currentClust)) {
					...
					similarNodes.put(op.getQName().getLocalPart() + "-O",
							output);
	...
	private ArrayList<String> flattenOperationTerms(Operation op) {
		...
		Hashtable<String, String> childrensInput = d1.internFlattenDataTypes(op
				.getInput().getElement());
		Hashtable<String, String> childrensOutput = d1
				.internFlattenDataTypes(op.getOutput().getElement());
